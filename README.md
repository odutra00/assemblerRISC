...
2. O programa que testará a microarquitetura será o: prog_avaliacao.asm. Este programa executa a seguinte funcionalidade:
   ![Equação](https://latex.codecogs.com/png.image?\dpi{110}\bg_white (MemDados[última posição \leftarrow \left(\sum_{i=0}^{n-1} Mem[i + \text{Grupo} \cdot \text{CteMemDados}(h)]\right) \cdot 255(d))
